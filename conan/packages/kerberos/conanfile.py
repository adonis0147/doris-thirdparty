...
class Krb5Conan(ConanFile):
    ...
    def requirements(self):
        if self.settings.os.value == 'Macos':
    ...
    def build(self):
        ...
        env_build.configure(unix_path(os.path.join(self.source_folder, "src")), self._get_args())
        env_build.make()
        env_build.install()
    ...
    def _get_args(self):
        ...
        if self.settings.os.value == 'Macos':
            args.append("--with-crypto-impl=openssl")
        return args
